			   	...
			   	<div class="layui-form-item" style="margin-top:5px;margin-left:5px">
				     <div class="layui-block">
				     	<div class="layui-input-inline">
				            <input type="text" class="form-control" name="role" id="role" placeholder="角色名称"/>
				        </div> 
...
<script type="text/javascript">   
    ...
    $('#table').bootstrapTable({
        ...
        url:"${base}/role/getRoleListData",//要请求数据的文件路径
	...
	function rowformater(value,row,index){
    	 ...
    	 buttons+='</button>'; 
     	 ...
     	 buttons+='<button class="layui-btn layui-btn-xs layui-btn-radius" onclick="addRole(\''+row.id+'\')" >';
    	 buttons+=		'<i class="layui-icon">&#xe66f;</i>权限';
    	 buttons+='</button>';       	     	
    ...
    $('#search_btn').click(function(){
        $('#table').bootstrapTable('refresh', {url: '${base}/role/getRoleListData'});
    })
    //tableHeight函数
...
<script type="text/javascript">
	...
	function add(){
		layer.open({
                   title: '新增角色',//Layer提供了5种层类型。可传入的值有：0（信息框，默认）1（页面层）2（iframe层）3（加  载层）4（tips层）,
                    type: 2,
                    area: ['90%', '70%'],
                    shade: 0.1, //遮罩透明度
                    fixed: false, //不固定
	...
	function edit(id){
		layer.open({
                   title: '修改角色',
                    ...
                    maxmin: true,
                    shadeClose: true,
                    content: '${base}/role/roleEdit?id='+id
                });
	}
</script>
	...
	function addRole(id) {
		layer.open({
                   title: '角色权限配置',
                    ...
                    maxmin: true,
                    shadeClose: true,
                    content: '${base}/role/addRolePermission?id='+id,
                    btn: ['确定', '取消'],
		 		    yes: function(index, layero){
				  	   var iframeWin = window[layero.find('iframe')[0]['name']];
					   iframeWin.saveAuthor();
				    },

			   	...
			   	<div class="layui-form-item" style="margin-top:5px;margin-left:5px">
				     <div class="layui-block">
				     	<div class="layui-input-inline">
				            <input type="text" class="form-control" name="${entityName}" id="${entityName}" placeholder="${remark?default()}名称"/>
				        </div> 
...
<script type="text/javascript">   
    ...
    $('#table').bootstrapTable({
        ...
        url:"${"$"}{base}/${entityName}/get${entityCamelName}ListData",//要请求数据的文件路径
        ...
        columns:[
			...
			<#list columns as col>
            {
                title:'${col.remark?default()}',
                field:'${col.propertyName?default()}',
                visible:true
            },
            </#list>
            </#if>
	...
	function rowformater(value,row,index){
		var buttons="";
    	 ...
    	 buttons+='</button>';
    	 buttons+='<button class="layui-btn layui-btn-xs layui-btn-danger layui-btn-radius" onclick="doDelete(\''+row.id+'\')">';
    	 buttons+='<i class="layui-icon">&#xe640;</i>删除';
    	 buttons+='</button>'; 
    	 buttons+='<button class="layui-btn layui-btn-xs  layui-btn-warm layui-btn-radius" onclick="detail(\''+row.id+'\')">';
    ...
    $('#search_btn').click(function(){
        $('#table').bootstrapTable('refresh', {url: '${"$"}{base}/${entityName}/get${entityCamelName}ListData'});
    })
    //tableHeight函数
...
<script type="text/javascript">
	//批量删除获取ids
	 ...
	 $('#remove').click(function () {
	    	 ...
	    	 $.ajax({
	             type: "post",
	             url: "${"$"}{base}/${entityName}/delete${entityCamelName}",
...
<script type="text/javascript">
	...
	function add(){
		layer.open({
                   title: '新增${remark?default()}',//Layer提供了5种层类型。可传入的值有：0（信息框，默认）1（页面层）2（iframe层）3（加  载层）4（tips层）,
                    type: 2,
                    area: ['100%', '100%'],
                    shade: 0.1, //遮罩透明度
                    fixed: false, //不固定
                    maxmin: true,
                    shadeClose: true,
                    content: '${"$"}{base}/${entityName}/${entityName}Add'
                });
	}
</script>
	...
	function edit(id){
		layer.open({
                   title: '修改${remark?default()}',
                    ...
                    maxmin: true,
                    shadeClose: true,
                    content: '${"$"}{base}/${entityName}/${entityName}Edit?id='+id
                });
	}
</script>
	...
	function detail(id){
		layer.open({
                   title: '${remark?default()}详情',
                    ...
                    maxmin: true,
                    shadeClose: true,
                    content: '${"$"}{base}/${entityName}/${entityName}Detail?id='+id
                });
	}
</script>

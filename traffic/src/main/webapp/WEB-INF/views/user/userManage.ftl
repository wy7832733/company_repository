...
<script type="text/javascript">   
    ...
    $('#table').bootstrapTable({
        ...
        url:"${base}/user/getUserListData",//要请求数据的文件路径
    ...
    $('#search_btn').click(function(){
        $('#table').bootstrapTable('refresh', {url: '${base}/user/getUserListData'});
    })
    //tableHeight函数
...
<script type="text/javascript">
	...
	function add(){
		layer.open({
                   title: '新增账号',//Layer提供了5种层类型。可传入的值有：0（信息框，默认）1（页面层）2（iframe层）3（加  载层）4（tips层）,
                    type: 2,
                    area: ['90%', '75%'],
                    shade: 0.1, //遮罩透明度
                    fixed: false, //不固定
	...
	function edit(id){
		layer.open({
                   title: '修改账号',
                    ...
                    maxmin: true,
                    shadeClose: true,
                    content: '${base}/user/userEdit?id='+id
                });
	}
</script>
